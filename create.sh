#!/bin/bash
#
# This source file is part of the VisionProSurgery based on the Stanford Spezi Template Application project
#
...
oldBundleIdentifierEscaped=$(sed 's:/:\\/:g' <<< "edu.stanford.vpsurgery")
...
oldProvisioningProfileEscaped=$(sed 's:/:\\/:g' <<< "\"VisionProSurgery\"")
...
sed -i '' "s/${oldProvisioningProfileEscaped}/${provisioningProfileEscaped}/g" "./VisionProSurgery.xcodeproj/project.pbxproj"
...
sed -i '' "s/${firebaseProjectNameEscaped}/${firebaseProjectEscaped}/g" "./VisionProSurgery/Supporting Files/GoogleService-Info.plist"


# Rename project and code:
projectNameLowercaseEscaped=$(sed 's:/:\\/:g' <<< "visionprosurgery")
headerFileEscaped=$(sed 's:/:\\/:g' <<< "VisionProSurgery based on the Stanford Spezi Template Application")
projectNameNoSpacesEscaped=$(sed 's:/:\\/:g' <<< "VisionProSurgery")
projectNameSpeziEscaped=$(sed 's:/:\\/:g' <<< "VisionProSurgery")
projectNameEscaped=$(sed 's:/:\\/:g' <<< "VisionProSurgery")
templateEscaped=$(sed 's:/:\\/:g' <<< "VisionProSurgery")
sstaEscaped=$(sed 's:/:\\/:g' <<< "Stanford Spezi Template Application")
taEscaped=$(sed 's:/:\\/:g' <<< "TemplateApplication")

sstaFullEscaped=$(sed 's:/:\\/:g' <<< "Stanford VisionProSurgery")
taFullEscaped=$(sed 's:/:\\/:g' <<< "VisionProSurgery")
